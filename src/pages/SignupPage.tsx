...
function SignupPage() {
  ...
  const handleGoogleSignup = async () => {
    ...
    try {
      ...
      console.log('User signed up/in with Google:', result.user);
      navigate('/');
    } catch (err: any) {
  ...
  return (
    <div className="flex items-center justify-center min-h-screen bg-primary font-sans">
      <div className="w-full max-w-md px-8 py-10 bg-white rounded-lg shadow-xl">
        ...
        <h3 className="text-2xl font-bold text-center text-neutral-dark mb-8">Create your account</h3>
        <form onSubmit={handleEmailSignup}>
          <div className="space-y-4">
            ...
            <div>
              ...
              <input type="password"
                     placeholder="Password (min. 6 characters)" id="password" value={password} onChange={(e) => setPassword(e.target.value)} required minLength={6} disabled={loading}
                     className="w-full px-4 py-2 mt-1 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-primary focus:border-primary disabled:bg-gray-100" />
            </div>
            <div>
              <label className="block text-sm font-medium text-neutral-dark" htmlFor="confirmPassword">Confirm Password</label>
              <input type="password"
                     placeholder="Confirm Password" id="confirmPassword" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} required disabled={loading}
                     className="w-full px-4 py-2 mt-1 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-primary focus:border-primary disabled:bg-gray-100" />
            </div>
        ...
        <div className="mt-6 text-center text-sm">
          <span className="text-neutral-dark/75">Already have an account?</span>
          <Link to="/login" className="font-medium text-primary hover:text-teal-light ml-1">
            Log in
          </Link>
        </div>
      </div>
